...
// Voice style data structure from .bin files
...
/**
 * Load Supertonic voice style from URL (downloads and caches)
 */
export async function loadVoiceStyleFromURL(voiceUrl: string, cacheDir?: string): Promise<Style> {
    const cachePath = cacheDir || path.join(process.cwd(), '.cache', 'voices');
    
    // Extract filename from URL
    const url = new URL(voiceUrl);
    const filename = path.basename(url.pathname);
    const localPath = path.join(cachePath, filename);
    ...
    if (!fs.existsSync(localPath)) {
        ...
        console.log(`Voice cached to ${localPath}`);
    }
...
export async function loadSupertonicONNX(onnxDir: string): Promise<SupertonicONNX> {
    ...
    return new SupertonicONNX(cfgs, processor, dp, textEnc, vector, vocoder);
}

export function BufferWav(audioData: number[], sampleRate: number): Buffer {
...
/**
 * Benchmark a TTS implementation
 */
export async function benchmarkTTS(
    implementation: string,
    ttsFn: () => Promise<{ wav: number[]; duration: number[] }>,
    sampleRate: number
    ...
    const inferenceTime = (endTime - startTime) / 1000; // in seconds
    const audioDuration = result.wav.length / sampleRate;
