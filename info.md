...
**2026.01.10** - 🎉 **Code Modularization Complete!** The code has been refactored into a clean, maintainable package structure with proper separation of concerns.
...
The code has been modularized into the following package structure:

```
src/main/java/supertonic/
├── ExampleONNX.java           # Main entry point
├── CommandLineArgs.java      # CLI argument parsing
├── model/
│   ├── Config.java           # TTS configuration models
│   ├── Languages.java        # Language validation
│   ├── Result.java           # TTS result holder
│   └── Style.java            # Voice style tensors
├── processor/
│   └── TextProcessor.java    # Text preprocessing & normalization
├── inference/
│   └── TextToSpeech.java     # Core TTS inference logic
└── util/
    ├── AudioUtils.java       # WAV file I/O utilities
    ├── ModelLoader.java      # Model & style loading
    └── TextChunker.java      # Long text chunking
```

### Package Descriptions

- **`supertonic.model`** - Data classes (Configuration, Result, Style)
- **`supertonic.processor`** - Text preprocessing and normalization
- **`supertonic.inference`** - Core TTS inference using ONNX Runtime
- **`supertonic.util`** - Utility functions (I/O, timers, etc.)
...
The system automatically chunks long texts into manageable segments, synthesizes each segment separately, and concatenates them with natural pauses (0.3 seconds by default) into a single audio file:
...
**Note**: Automatic text chunking is disabled when using `--batch` mode.
